...
from MinatoNamikaze import (EVENT_LOGS, LOGGER, OWNER_ID, SUDO_USERS, WHITELIST_USERS,
                          dispatcher)
...
from MinatoNamikaze.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @MinatoNamikazeSupport should this problem persist!"
            )
            return
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if fban:
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @MinatoNamikazeSupport."
            )
            return
...
def set_frules(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @MinatoNamikazeSupport !"
            )
            return
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Minato_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Minato_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Minato_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Minato_fbanned_users.csv",
