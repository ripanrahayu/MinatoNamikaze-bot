...
from MinatoNamikaze import (DEV_USERS, OWNER_ID, SUDO_USERS, SUPPORT_USERS, WHITELIST_USERS,
                          INFOPIC, dispatcher, sw)
from MinatoNamikaze.__main__ import STATS, TOKEN, USER_INFO
...
from MinatoNamikaze import telethn as MinatoTelethonClient, SUDO_USERS, SUPPORT_USERS
...
@MinatoTelethonClient.on(
    events.NewMessage(
        pattern='/ginfo ',
        from_users=(SUDO_USERS or []) + (SUPPORT_USERS or [])))
...
def info(update: Update, context: CallbackContext):
    ...
    if disaster_level_present:
        text += ' [<a href="https://t.me/NamikazeMinatoChannel/54">?</a>]'.format(
            bot.username)
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "{}Information about the him has been successfully collected !")
            return
